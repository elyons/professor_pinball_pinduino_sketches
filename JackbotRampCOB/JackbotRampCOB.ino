...
void checkPinStates(){
  ...
  if ( pd.pinState()->J126(1) ){ // center visor flasher
    pd.adrLED1()->clear();
    pd.adrLED1()->RGBBand(4, 0,0,255, 4);
    delay(200);
    pd.adrLED1()->clear();
    pd.adrLED1()->RGBBand(11, 255,0,0, 4);
    delay(200);
    trigger = 1; 
  }
  ...
  if ( pd.pinState()->J126(4) ){ // lower left flasher
    pd.adrLED1()->bullet("red", 3, 20,  1);
    trigger = 1;
  }

  if ( pd.pinState()->J126(5) ){ // middle left flasher
    pd.adrLED1()->bullet("blue", 3, 20,  1);
    trigger = 1; 
  }
  
  if ( pd.pinState()->J126(6) ){ // lower right flasher
    pd.adrLED1()->bullet("purple", 3, 20,  -1);
    trigger = 1; 
    trigger = 1; 
  }
  ...
  if ( pd.pinState()->J126(16) ){ // visor motor
    ...
    pd.adrLED1()->bullet2Color("red", "blue", 10, 20, -1);
    trigger = 1;
  }
...
//trigger is to take care of any cleanup after a sequence has been triggered.
  if (trigger) {
   ...
   pd.adrLED1()->colorRGB(125,125,125);
  }
